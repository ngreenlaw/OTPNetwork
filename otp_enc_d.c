/*Nathan Greenlaw
 * CS 344
 * Program 4: otp_dec_d encrypt text with a given key by receiving it from otp_enc*/
...
while(1)
{
	...
	// Get the message from the client and loop until all data is received
	...
	// this is going to be the input text termminated by **
	char message[71000];
	memset(message,'\0',sizeof(message));
	
	// this is going to be the key terminated by @@
	...
	int le = strlen("**");
	//move just the message into the message variable
	...
	//encrypt the message using the key by the otp method
	...
	for(j;j<len;j++)
	{
		...
		if(mj == 32) // if a space, make it the 26th character
		{
			mj = 26;
		}
		else
	...
	//send the encrypted text back and loop until all is sent
	int charsWritten;
	int curBuffer = 0;
