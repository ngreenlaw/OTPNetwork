/*Nathan Greenlaw
 * CS 344
 * Program 4: otp_enc encrypt text with a given key by sending it to otp_enc_d*/
...
//get the file size to set up buffer
	...
	// strip newline and error check the entered file
	int i=0;
	...
	//send code to server to ensure it is the correct one
	/*
	int correct;
	char code[20];
	...
	//Send the entire message to the server to parse it for the data
	int chrs;
	int cBuffer = 0;
	...
	//Loop until all of the encrypted data is received and remove the terminal character
	char echeck[71000];
